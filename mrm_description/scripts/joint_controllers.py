...
/mrm/joint3_position_controller/command
/mrm/joint4_position_controller/command
/mrm/joint5_position_controller/command
/mrm/joint6_position_controller/command
...
class gurdyJointMover(object):
    ...
    def __init__(self):
        ...
        rospy.loginfo("Mrm JointMover Initialising...")

        self.pub_joint1_position_controller = rospy.Publisher(
            '/mrm/joint1_position_controller/command',
            Float64,
            queue_size=1)
        self.pub_joint2_position_controller= rospy.Publisher(
            '/mrm/joint2_position_controller/command',
            Float64,
            queue_size=1)
        self.pub_joint3_position_controller = rospy.Publisher(
            '/mrm/joint3_position_controller/command',
            Float64,
            queue_size=1)
        self.pub_joint4_position_controller = rospy.Publisher(
            '/mrm/joint4_position_controller/command',
            Float64,
            queue_size=1)
        self.pub_joint5_position_controller = rospy.Publisher(
            '/mrm/joint5_position_controller/command',
            Float64,
            queue_size=1)
        self.pub_joint6_position_controller = rospy.Publisher(
            '/mrm/joint6_position_controller/command',
            Float64,
            queue_size=1)
        joint_states_topic_name = "/mrm/joint_states"
        rospy.Subscriber(joint_states_topic_name, JointState, self.mrm_joints_callback)
        mrm_joints_data = None
        rate = rospy.Rate(2)
        while mrm_joints_data is None:
            try:
                mrm_joints_data = rospy.wait_for_message(joint_states_topic_name, JointState, timeout=5)
            except:
                rospy.logwarn("Time out " + str(joint_states_topic_name))
                pass
            rate.sleep()

        self.mrm_joint_dictionary = dict(zip(mrm_joints_data.name, mrm_joints_data.position))

    def move_mrm_all_joints(self, upperlegM1_angle, upperlegM2_angle, upperlegM3_angle, lowerlegM1_value, lowerlegM2_value ,lowerlegM3_value):
        upperlegM1 = Float64()
